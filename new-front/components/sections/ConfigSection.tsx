...
import { BachSchedule, CourseStructure, GroupConfigMap, ToponConfigMap } from '@/lib/types';

export type ConfigSectionProps = {
  courses: { asig_codigo: string; asig_nombre: string }[];
  configCourse: string;
  configSection: string;
  configGroups: number[];
  availableGroups: number[];
  courseStruct: CourseStructure[];
  ...
  setConfigSection: (val: string) => void;
  ...
  setToponSelected: (val: string) => void;
};
              ...
              {availableGroups.map(g => {
                ...
                return (
                  <button
                    ...
                    className={`px-3 py-1 rounded-md border text-sm ${active ? 'bg-indigo-600 text-white border-indigo-400' : 'bg-slate-900 text-slate-200 border-slate-700'}`}
                  >
                    Grupo {g}
                  </button>
                );

...
export default function HomePage() {
  ...
  const removeGroupConfig = (key: string) => {
    setGroupConfigs(prev => { const copy = { ...prev }; delete copy[key]; return copy; });
  };
  ...
  const removeToponConfig = (key: string) => {
    setToponConfigs(prev => { const copy = { ...prev }; delete copy[key]; return copy; });
  };
  ...
  const pollMassiveState = async () => {
    try {
      ...
      if (state.done) {
        ...
        setMassSummary(state.summary || null); setMassResults(state.results || []);
        setCapacityReport(state.capacity_report || null); setCapacityStats(state.capacity_stats || null);
  ...
  const applyMassFilters = (text: string, status: string) => {
    ...
    setMassFiltered(filterMassResults(massResults, text, status));
  };
  ...
  const massSave = async () => {
    ...
    try {
      ...
      setMassResults(prev => {
        ...
        setMassSummary(recomputeMassSummary(updated));
        setMassFiltered(filterMassResults(updated, massFilterText, massFilterStatus));
        return updated;
      });
